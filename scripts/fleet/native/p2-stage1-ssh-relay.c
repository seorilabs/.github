...
static int command_allowed(const char *node, const char *command) {
  ...
  static const char *patterns[] = {
    ...
    "^sudo -S -p '' /bin/sh -c 'exec /usr/local/libexec/seori-auth-native launch -- /usr/local/bin/node /opt/seorilabs/fleet-p2/[a-f0-9]{40}/scripts/fleet/provision-p2-host-encryption\\.mjs readback --kubeconfig=/var/snap/microk8s/current/credentials/kubelet\\.config --tang-attestation=/var/lib/seorilabs/tang-backup-attestations/rpi4001\\.json --tang-attestation=/var/lib/seorilabs/tang-backup-attestations/seori-m6-01\\.json --public-error-channel=stdout 3</dev/null </dev/null'$",

...
`@seorilabs/repo-contract/fleet-migration`은 P7 legacy 운영 JSON, `secrets: inherit`, floating 중앙 workflow ref를 서명된 전체 GitHub App pagination inventory와 exact commit/tree/BLOB readback에 묶어 분류합니다. source repository와 P5 classification revision으로 확정한 PRODUCT_APP subject를 분리하되, cross-repo는 `PLATFORM_REGISTRY_APP`과 ACTIVE App/PlatformFleetBinding revision·digest readback이 모두 일치할 때만 허용합니다. fork, 중복 registry-to-app mapping, 모호한 subject는 fail-closed합니다. ACTIVE config/signed snapshot/replacement에 묶인 build-only, 검증 시각 기준 authoritative parity head/total/TTL, replacement가 정한 CredentialBinding provider/capability/environment/public identity/policy revision과 scope/generation, consumer 0/parser disabled/dispatch readback, exact-tree Git restore가 모두 일치해야 합니다. 최초 권위 inventory 전에 적용된 Platform security remediation은 `PRE_AUTHORITATIVE_SECURITY_REMEDIATION` ratification으로 detector/cohort/commit/BLOB 전이와 38/73/107/86을 고정합니다. 이 ratification은 runtime ID·digest·서명·timestamp를 포함하지 않고 collector와 issuer가 생성한 inventory digest 및 Ed25519 payload에 결합합니다. historical cohort는 Backoffice `f68b044263422bb1a25785faac864f86557d3d4f`의 `fleet-migration-shadow-readiness/v2`와 같은 canonical vector—installation ID와 active repository 38개의 numeric ID 순 ID/full name/default branch/archived/private/fork/exact HEAD—에서 재계산하며 `6b940f78bf810b5f725ff6c2d71af14fe2127d0de98c893e180594eeae29460d`와 exact 일치해야 합니다. 자기 참조를 막기 위해 historical digest 계산에서만 detector repository `1241442018`의 HEAD를 ratification의 `cd13b325918cb10401e089074461ba11042c154e`로 정규화합니다. 현재 `.github` default HEAD는 trusted runtime `detectorSourceSha`와 별도로 exact 대조하고 actual SHA를 inventory, detection, provider vector와 서명 payload에 결합하므로 다른 repository drift나 claimed/live detector 불일치는 숨길 수 없습니다. schemaVersion 1에서 ratification 필드는 구조 호환을 위해 optional이지만, 모든 authoritative current inventory와 checkpoint의 exact ratification을 요구합니다. 최초 ratified 수치는 `BOOTSTRAP`에만 쓰며 후속 `WAVE`는 `--prior-inventory`의 신뢰 digest를 잇고 cleanup count를 단조 감소시킵니다. 각 WAVE inventory는 BOOTSTRAP부터 직전 WAVE까지의 서명된 compact checkpoint와 누적 `chainDigest`를 포함합니다. WAVE chain head는 별도 state authority가 durable CAS로 exact current generation/head에서 candidate inventory ID/digest/signedAt으로 확보한 단일 reservation이어야 합니다. loader는 signed artifact만 믿지 않고 주입된 `trustedStateAuthorityReadback`에서 현재 reservation 전체를 다시 읽어 대조하므로, state가 진행된 뒤의 old head와 동일 parent sibling은 binding을 얻지 못합니다. 이 adapter가 없으면 standalone CLI도 `FLEET_MIGRATION_STATE_AUTHORITY_READBACK_REQUIRED`로 fail-closed합니다. inventory signer와 chain-head authority의 key ID와 SPKI는 WAVE 검증 시 합쳐진 inventory trust set에서도 다시 분리합니다. 출력은 항상 `PLAN_ONLY`·stdout 전용이며 repository write, PR 생성, 파일 삭제·rewrite API를 제공하지 않습니다. CLI의 모든 outcome 의미 검증에는 `--inventory`, `--trusted-key-id`, `--trusted-public-key`가 필요하고 wave에는 `--prior-inventory`, `--chain-head`, `--trusted-chain-head-key-id`, `--trusted-chain-head-public-key`와 trusted live readback adapter도 필요합니다. `validateFleetMigrationPlanStructure`는 구조만 확인하며 권위 검증이 아닙니다. 실제 durable CAS 저장소와 provider transport는 주입되는 운영 adapter의 책임이며 `planDigest` 자체는 권한이 아닙니다.

`@seorilabs/repo-contract/fleet-migration-collector`는 read-only GitHub App pagination, exact default HEAD와 non-truncated 전체 tree digest를 수집하고 detector 관련 legacy manifest/JSON과 workflow BLOB만 제한해 읽습니다. Backoffice App·ACTIVE ConfigRevision·verified signed snapshot·PlatformFleetBinding·ProviderObservation·CredentialBinding 공개 readback은 같은 durable occurrence에 결합합니다. permission/network/readback 오류를 부재로 바꾸지 않고, truncated tree·source drift·non-canonical BLOB·candidate substitution을 차단합니다. `@seorilabs/repo-contract/trusted-inventory-issuer`는 durable collection을 다시 읽은 뒤 live GitHub App identity, accepted permission union, webhook URL, event union과 `repository` event acceptance가 exact할 때만 `READY` Ed25519 attestation을 발급합니다. 이 gate는 이슈 번호나 상태를 보지 않으므로 외부 승인 후 코드 변경 없이 readback으로 열리며, 현재 capability가 부족하면 `GITHUB_APP_CAPABILITY_UNVERIFIED`로 닫힙니다. issuer는 `shared/platform/fleet-release-approval-signing` logical ID, `platform-fleet-release-20260829-5458c56b` public key ID, `FLEET_MIGRATION_INVENTORY_ATTESTATION` purpose와 supplied SPKI fingerprint의 최신 public metadata readback을 exact하게 검증합니다. worker와 signer는 logical ID와 canonical payload/digest만 교환하며 private key 입력·반환 API는 없습니다. 실제 GitHub/Backoffice transport, durable store와 signing service 구현은 주입되는 trusted adapter의 책임입니다.

`@seorilabs/repo-contract/trusted-cleanup-executor`는 issuer가 발급한 authoritative `BOOTSTRAP` inventory와 같은 프로세스에서 다시 만든 trusted inventory binding, `PLAN_ONLY` plan을 매 실행과 각 mutation 직전에 재검증합니다. issue의 `autopilot`·승인 scope/TTL을 확인하고 `blocked`, `no-autopilot`, `approval:*`를 거부하며, exact numeric repo ID/full name/default HEAD/tree/BLOB과 열린 autonomous Ready PR 수를 readback합니다. 외부 state authority의 generation과 chain-head binding을 CAS로 한 번 reserve한 뒤 한 repo의 `CREATE_COMMIT → CREATE_REF → CREATE_PR` ledger만 진행합니다. mutation은 plan에 고정된 삭제와 exact replacement digest만 새 branch에 반영하고 default branch 변경·ref 삭제·Draft PR은 API surface에서 허용하지 않습니다. 각 단계는 provider readback-first로 재개하며 한 번 `DISPATCHED`된 mutation은 이후 readback이 `ABSENT`여도 반복하지 않습니다. 완료 직전 exact commit, branch ref, 열린 Ready PR과 deterministic operation ID를 다시 검증한 뒤 최종 receipt와 stale generation을 exact consume합니다. adapter와 receipt는 공개 필드만 허용하고 lease token은 state adapter 경계 안에서만 복제·폐기합니다. 이 모듈은 실행 core와 schema이며 실제 GitHub/Backoffice transport, durable state implementation, live shadow 실행 또는 legacy 파일 삭제 완료를 주장하지 않습니다.

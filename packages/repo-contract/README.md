...
`@seorilabs/repo-contract/fleet-migration`은 P7 legacy 운영 JSON, `secrets: inherit`, floating 중앙 workflow ref를 서명된 전체 pagination inventory와 exact repository ID/ref/SHA/git blob에 묶어 분류합니다. 출력은 항상 `PLAN_ONLY`·stdout 전용이며 repository write, PR 생성, 파일 삭제·rewrite API를 제공하지 않습니다. ACTIVE config/market readback, latest contiguous parity 2회, 승인된 WorkflowBundle build-only, Git 및 Backoffice 장애 복구, owner/control-plane readback이 모두 일치해도 결과는 `READY_FOR_REVIEW`까지만 올라갑니다. `planDigest`는 권한이 아니므로 READY 검증에는 외부 신뢰키로 검증한 원본 inventory binding이 다시 필요합니다.

...
`providerControlPlane`은 exact `backofficeClientSpiffeId`와 고정
`endpointScope=/internal/control-plane/provider-grants` 두 필드만 가지며 runtime config,
container startup argument, Pod annotation이 모두 같아야 합니다.

...
test('production Secret Manager child reconciles an ambiguous write without exposing the value', async () => {
  const root = await realpath(await mkdtemp(join(tmpdir(), 'seori-auth-secret-verify-')));
  const wrapper = join(root, 'gcloud-cli.sh');
  const token = 'test-access-token-0123456789-abcdefghijklmnopqrstuvwxyz';
  const secret = randomBytes(32);
  ...
  const server = createServer((request, response) => {
    ...
    response.end(JSON.stringify({
      name: `${resource}/versions/1`,
      payload: {
        data: secret.toString('base64'),
        dataCrc32c: crc32c(secret),
      },
    }));
  });
  ...
  try {
    ...
    assert.deepEqual(result, {
      schemaVersion: 1,
      operation: 'secret-version-verify',
      resourceName: resource,
      versionResourceName: `${resource}/versions/1`,
      dataCrc32c: crc32c(secret),
      backupRestoreVerified: true,
...
test('production Secret Manager child rejects a recovered version with different material', async () => {
  const root = await realpath(await mkdtemp(join(tmpdir(), 'seori-auth-secret-mismatch-')));
  const wrapper = join(root, 'gcloud-cli.sh');
  const token = 'test-access-token-0123456789-abcdefghijklmnopqrstuvwxyz';
  const localSecret = randomBytes(32);
  const remoteSecret = randomBytes(32);
  const resource = 'projects/seorilabs-ci/secrets/seori-auth-canary-password';
  const server = createServer((request, response) => {
    response.writeHead(200, { 'content-type': 'application/json' });
    response.end(JSON.stringify({
      name: `${resource}/versions/1`,
      payload: {
        data: remoteSecret.toString('base64'),
        dataCrc32c: crc32c(remoteSecret),
      },
    }));
  });
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  try {
    ...
    child.stdio[3].end(localSecret);
    const [resultText, completion] = await Promise.all([
      new Response(child.stdio[5]).text(),
      new Promise((resolve, reject) => {
        child.once('error', reject);
        child.once('close', (code, signal) => resolve({ code, signal }));
    ...
    assert.deepEqual(completion, { code: 65, signal: null });

...
try {
  const [helperSha256, nodeSha256, childSha256] = await Promise.all([
    sha256(helperPath), sha256(nodePath), sha256(childPath),
  ]);
  ...
  try {
    await boundary.secretManagerAccessor({ nodeSha256, childSha256: '0'.repeat(64) });
  } catch (error) {
    digestMismatchBlocked = error instanceof SeoriAuthError && error.code === 'native_helper_mismatch';
  }
  const accessor = await boundary.secretManagerAccessor({ nodeSha256, childSha256, timeoutMs: 2_000 });

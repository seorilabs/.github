...
## readback만으로 초기화한 원장의 첫 릴리스

provider readback으로 baseline을 정한 원장은 번호 이력은 있지만 **태그 이력이 없다**
(`release.lastTag: null`). 2026-09-16 이관에서 15개 중 11개가 이 상태였다.

이 상태에서는 태그를 명시하지 않은 릴리스가 `ledger-last-tag-unknown` 으로 멈춘다.

```
원장에 release.lastTag가 없어 bump할 기준이 없다. 이 저장소의 첫 릴리스는 태그를 명시해야 한다.
```

없는 값을 `v0.0.0` 으로 대신하면 이미 `v1.10.3` 까지 나간 앱에 `v0.0.1` 을 만든다. 번호는 원장이
정상적으로 올려주므로 겉으로는 성공한 것처럼 보이고, 그 태그가 다시 `lastTag` 가 되어 이후
bump까지 오염시킨다. 그래서 추측하지 않고 멈춘다.

**이관 후 첫 릴리스는 태그를 명시한다.** 한 번 명시하면 그 값이 `lastTag` 가 되어 이후에는
bump가 정상 동작한다.

...
| (없음) | — | — | — | App Store 트리거는 Backoffice 가 ASC `ciBuildRuns` 로 직접 한다. GitHub Actions 에 App Store 경로를 두지 않는다 |
...
| (없음) | — | — | — | App Store 트리거는 Backoffice 가 ASC `ciBuildRuns` 로 직접 한다 |
...
`rn-build-ait.yml`은 마켓 승인 전에도 후보 artifact를 검증할 수 있는 build-only
경로다. workflow 이름과 결과는 deploy 신호로 취급하지 않으며 AppsInToss environment,
WIF, API 업로드와 명확히 분리한다.

Android 후보는 별도 워크플로를 두지 않는다. `rn-deploy-google-play.yml`을
`upload: false`로 부르면 서명 AAB를 Actions artifact로만 남긴다.
...
- **Godot 게임 애널리틱스(repo)**: GODOT_ANALYTICS_CONFIG_JSON_BASE64 — `<project_dir>/analytics.config.json`을 직접 소비하는 앱의 GA4 Measurement Protocol 설정 base64. `godot-deploy-{ait,google-play}` 재사용 워크플로우가 값이 있을 때만 Godot import 전에 복원한다. 미설정은 파일 주입 생략일 뿐이며, Platform ingest 중계처럼 앱이 자체 분석 경로를 가진 경우 정상이다. Godot Pages(웹 프리뷰)는 스트림 오염 방지를 위해 주입 제외.

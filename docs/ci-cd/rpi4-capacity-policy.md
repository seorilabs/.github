...
- 일반 ARC는 `minRunners: 1`, `maxRunners: 3`, DIND는 `minRunners: 0`,
  `maxRunners: 1`이다. 선언값, AutoscalingRunnerSet spec 또는 current/pending/running
  수가 상한을 벗어나면 검증 실패다.
...
4. scheduler, ARC, Auth Broker의 exact RPI5 binding과 ARC `1/3`, `0/1` 유지

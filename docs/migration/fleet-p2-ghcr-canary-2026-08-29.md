...
PVC를 참조하지 않는다. RPI5, no ServiceAccount token, read-only root filesystem, non-root UID/GID,
RuntimeDefault seccomp, capabilities `ALL` drop, tmpfs runtime/state, ingress/egress deny,
`backoffLimit: 0`이 고정된다.

Job 이름과 full idempotency annotation은 image digest, source provenance, registry binding,
canary contract version의 canonical SHA-256에서 파생한다. 기존 Job은 완료·실패·결과 불명 모두
재실행하지 않고 readback한다. 성공 output은 아래 한 줄의 hash와 exact-match해야 한다.
...
이번 변경은 manifest 생성과 검증 계약까지만 구현하며 package visibility, credential, GCP/IAM,
Kubernetes live state를 변경하지 않는다.

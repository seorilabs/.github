...
const hostBackupState = /provision-p2-host-encryption\.mjs backup-state --kubeconfig=\/var\/snap\/microk8s\/current\/credentials\/client\.config --public-error-channel=stdout 3<&0'$/u
  .exec(remoteCommand);
...
const hostBackup = /provision-p2-host-encryption\.mjs backup --confirmation=fleet-p2-host-backup-[a-f0-9]{12} --kubeconfig=\/var\/snap\/microk8s\/current\/credentials\/client\.config --public-error-channel=stdout 3<&0'$/u
  .exec(remoteCommand);
if (hostBackup !== null) {
  if (nodeName !== contract.target.nodeName) process.exit(126);
...
const hostEncryptionReadback = /provision-p2-host-encryption\.mjs readback --kubeconfig=\/var\/snap\/microk8s\/current\/credentials\/client\.config --tang-attestation=\/var\/lib\/seorilabs\/tang-backup-attestations\/rpi4001\.json --tang-attestation=\/var\/lib\/seorilabs\/tang-backup-attestations\/seori-m6-01\.json --public-error-channel=stdout 3<&0'$/u
  .exec(remoteCommand);

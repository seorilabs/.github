...
- 러너: `release-tag.yml`과 `promote-google-play.yml`은 private에서 `seorilabs-x64`,
  public에서 `ubuntu-latest`로 중앙이 판정한다. Godot Play와 RN Play은 `ubuntu-latest`로
  고정한다. 어느 쪽도 caller가 러너를 선택하는 `runs_on` 입력은 없다.
